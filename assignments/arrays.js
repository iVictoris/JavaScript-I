...
// yes I know var really shouldn't be used but lazy and wanted to override var names
var { car_year, car_make, car_model } = inventory.filter(c => c.id === 33)[0]; 
...
var {length} = inventory
var {car_make, car_model} = inventory[length-1]
// let lastCar = 0; 
console.log(car_make, car_model);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. 
// Sort all the car model names into alphabetical order and 
// log the results in the console
// * Was going to overly complicated, but then I thought I could just use sort.
// let carModels = inventory.map(cars => cars.car_model).sort();
// let carModelsSorted = carModels.map(make => {

// });
let carModels = inventory.sort(({car_make: aMake}, {car_make: bMake}) => {
  return aMake > bMake ? 1 : aMake < bMake ? -1 : 0;
})
...
let carYears = inventory.map(({car_make}) => car_make);
console.log();
...
let oldCars = carModels.filter(year => year < 2000);
